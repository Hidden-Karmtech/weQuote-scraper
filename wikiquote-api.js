...
var wikiquoteApi = (function() {
	...
	var API_URL = "http://it.wikiquote.org/w/api.php";
	...
	wqa.queryTitles = function(titles, success, error) {
		...
		function (error, response, result) {
			if (error)
				console.log("Error processing your query"); 
			else
			{
		...
		function (error, response, result) {
			...
			else
			{
				...
				for(var s in sections) {
					if ((sections[s].line!=="Bibliografia")&&
						(sections[s].line!=="Note")&&
						(sections[s].line!=="Altri progetti")&&
						(sections[s].line!=="Voci correlate"))
					sectionArray.push(sections[s].number);
				...
				console.log(result.parse.title+"=>Sections: "+sectionArray);
					...
					lis.each(function() {
						...
						if($bolds.length > 0) {
							...
							if (($(this).text()!=="")&&
								($(this).text()!=="\n\n")&&
								($(this).text()!==" ")&&
								(isNaN(parseInt($(this).text().charAt(0)))))	
								{
								quoteArray.push(
										{
											"quote":$(this).text().replace(/ *\([^)]*\) */g, ""),
											"author":result.parse.title
										});
								console.log("get quote n. " + quoteIndex++);
								}
						}
					});
